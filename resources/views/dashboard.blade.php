...
<div class="max-w-7xl mx-auto space-y-8">
    ...
    <header class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex justify-between items-center">
        ...
        <div class="text-right">
            <p class="font-bold text-gray-800">Halo, {{ auth()->user()->name }}</p>
            <p class="text-sm text-gray-500">{{ auth()->user()->email }} &middot; {{ auth()->user()->role }}</p>
            <div class="mt-2 flex gap-3 justify-end text-sm font-semibold">
                <a class="text-indigo-600 hover:underline" href="{{ route('task-lists.index') }}">Daftar Tugas</a>
                <a class="text-indigo-600 hover:underline" href="{{ route('tasks.index') }}">Tasks</a>
                @if (auth()->user()->role === 'admin')
                    <a class="text-indigo-600 hover:underline" href="{{ route('admin.users.index') }}">Manage Users</a>
                @endif
            </div>
            <form method="POST" action="{{ route('logout') }}" class="mt-2">
                @csrf
                <button type="submit" class="text-sm text-red-600 font-semibold hover:underline">Keluar</button>
            </form>
        </div>
    </header>
